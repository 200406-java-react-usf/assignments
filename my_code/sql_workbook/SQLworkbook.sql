...
-- 3.2 System Defined Aggregate Functions --
	...
	-- Task – Create a function that returns the most expensive track
		create or replace function public.MaxPrice() returns setof "Track" as $$ begin
						return Query select * from "Track" where "UnitPrice" = 
					(select "UnitPrice" from "Track" order by "UnitPrice" desc limit 1); end; $$ language plpgsql;
...
-- 4.1 Basic Stored Procedure --
	-- Task – Create a stored procedure that selects the first and last names of all the employees.
		create procedure SelectEmployeeNames AS select "FirstName", "LastName" from "Employee" go;
