...
window.onload = function() {
    ...
    // Add an EventListener to id and select fields for validating input
    ID_FIELD.addEventListener('blur', fieldsValid);
    SELECT_FIELD.addEventListener('blur', fieldsValid);

    // Add an EventListener to submit button to send a req to the SWAPI
    SUBMIT_BTN.addEventListener('click', fetchInfo);

    // Disable the submit button by default
    SUBMIT_BTN.setAttribute('disabled', true);

    // Hide alert message by default
...
function getInfo() {
    ...
    /*
        AJAX Workflow
            Making asynchronous calls to an externaal web server using AJAX:
                1) Create an XMLHttpReuqest object (XHR)
    ...
    */
    ...
    // Step 3
    // xhr.send(JSON.stringify(body)); this is what it would look like for step 3 if we were making a post or put or something

    // Step 4
    xhr.onreadystatechange = () => {
        /* 
            The XMLHttpRequest.readyState property returns the state an XHR client is currently in. An XHR client exists in one of the following states

                0 - UNSENT; the XHR object has been created but .open() has not been called.
                1 - OPENED; .open() has been called
                2 - HEADERS_RECIEVED; .send() has been called AND the response headers and status code are available
                3 - LOADING; beginning to recieve the response body, xhr.responseText() would hold partial data
                4 - DONE; the operation is complete and the response is fully recieved
        */

        if (xhr.readyState === 4 && xhr.status === 200) {
            RESULTS_CONTAINER.innerText = xhr.responseText; // if you are getting XML you can use xhr.responseXML instead of responseText
        }
    }
